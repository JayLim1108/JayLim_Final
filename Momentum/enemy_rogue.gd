...
# [필수] 인스펙터에서 EnemyArrow.tscn 연결
@export var arrow_scene: PackedScene 
# [필수] 활/석궁 끝 Marker3D 노드 연결 (경로 확인!)
@onready var fire_point = get_node_or_null("Rig_Medium/Skeleton3D/BoneAttachment3D/Crossbow/FirePoint")

var attack_range = 15.0 # 원거리 공격 사거리 (Mage보다 길게 설정 가능)
...
func shoot_arrow():
	if arrow_scene:
		...
		get_parent().add_child(arrow) # 씬(Main)에 추가
		
		# 발사 위치 및 방향 설정
		if fire_point:
			arrow.global_position = fire_point.global_position
			# 플레이어 가슴 높이 정도를 조준
			arrow.look_at(player.global_position + Vector3(0, 1.0, 0), Vector3.UP)
		else:
